...
export function PresetsSettings() {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 relative">
      ...
      <div className="mb-6 flex w-full space-x-1 overflow-x-auto rounded-lg border border-[#222226] bg-[#141417] p-1 sm:w-fit">
        {["系统预设", "我的预设"].map((tab) => (
          <button
            key={tab}
      ...
      <div className="grid grid-cols-1 gap-4 min-[380px]:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        {presets.map((style, i) => (
          <div key={i} className="lh-card group rounded-xl border border-border overflow-hidden hover:border-primary/50 transition-colors">
      ...
      {showEditModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-3 backdrop-blur-sm animate-in fade-in sm:p-4">
          <div className="lh-card flex max-h-[92vh] w-full max-w-lg flex-col rounded-2xl border border-border bg-card shadow-2xl">
            ...
            <div className="flex-1 space-y-5 overflow-y-auto p-4 sm:p-6">
              ...
              <div>
                ...
                <div className="flex gap-3">
                  ...
                  <div className="border-2 border-dashed border-border rounded-md flex flex-col items-center justify-center bg-layer-4 text-muted-foreground cursor-pointer hover:border-primary/50 h-16 w-16">
            ...
            <div className="flex shrink-0 flex-col justify-end gap-3 rounded-b-2xl border-t border-border p-4 sm:flex-row sm:p-5">
              <Button variant="outline" onClick={() => setShowEditModal(false)} className="border-border text-foreground hover:bg-accent">取消</Button>
              <Button onClick={() => setShowEditModal(false)}>保存预设</Button>
            </div>
          </div>
        </div>
      )}
